...
def _split_windows(
    ...
    without bars on both sides. A split is taken at its latest revision
    (#108): a re-dated split drops the price on its final ex-date, not the
    first one, and a split whose latest revision is cancelled never drops
    ...
    for group in _by_key(history).values():
        ...
        if isinstance(record, Bar):
            ...
            if isinstance(latest, CorporateAction) and not latest.cancelled:
