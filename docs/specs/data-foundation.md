...
5. **Corporate actions.** `PriceSource.corporate_actions()` supplies splits and dividends. **First-seen** row: `known_at` = source announcement time if present, else the close of the session before ex-date. The announcement time is stored as `announced_at` (NULL when the source gives none), so the rule is checkable: `known_at == announced_at` if set, else exactly the proxy; an earlier stamp without an announcement is look-ahead and refused (#83). A source that gives only an announcement **date** is stamped at the close of the first session strictly after that date (the announcement may follow that day's close). **Any later row that differs** for the same (security, type, ex-date): `known_at = ingested_at`. Never back-dated on revision.
