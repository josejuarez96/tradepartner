...
**Amendment 2026-09-25 (#216).** For XBRL filings since 2009 the SIC comes from the SEC Financial Statement and Notes data sets (`sub.sic`, as of the filing date) and the cover facts (`dei:Security12bTitle`, `dei:SecurityExchangeName`, `dei:TradingSymbol`, per-class `EntityCommonStockSharesOutstanding`) from the same files; the SGML header and the iXBRL document remain the sources for filings the data sets do not hold. Both are stamped at the filing's acceptance from the submissions cache, never from the data set's own `accepted` or `filed` columns. See plan T11c to T11e and [research #174](../research/2026-09-25-cover-page-facts-at-scale.md).
