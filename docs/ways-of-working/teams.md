...
- **Folding.** `uv run python scripts/fragments.py fold` moves every fragment into the two files in issue order and deletes it. It runs inside a PR that already edits those files for another reason (`doc-keeper` refreshing the snapshot sections, a plan amendment, the phase-close PR). Never open a PR just to fold: that is the bookkeeping-only PR the Phase 1 retro banned.
