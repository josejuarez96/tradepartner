...
- [ ] **T5: Fixture universe (CSV) and generator.** Files: `scripts/make_fixture_universe.py`, `tests/fixtures/universe/*.csv`, `tests/fixtures/universe/README.md` · Tests: `tests/test_fixture_universe.py` (every case in spec req 13 present; content-equal regeneration) · Depends on: T4 · Review: quant-auditor.
...
- [ ] **T20: `Broker` interface and fake broker.** Files: `src/tradepartner/adapters/{broker,fake_broker}.py` · Tests: `tests/adapters/test_fake_broker.py` · Depends on: T4 · Review: safety-reviewer. Runs in parallel with T5–T19.
