...
- [ ] **T21a: Dashboard shell.** Files: `src/tradepartner/dashboard/{__init__,app}.py` (Streamlit app entry; short-lived read-only connection via `store.db`; "store busy" state when the store is locked; navigation with an empty health-page placeholder in `app.py`, which T21 points at `health_page.py`) · Tests: `tests/dashboard/test_app.py` (headless render; read-only connection; busy state when locked) · Depends on: T4 · Review: `/code-review`. Runs in parallel with T5–T20, so UX work starts early (#53).
- [ ] **T21: Data-health page.** Files: `src/tradepartner/dashboard/health_page.py` · Tests: `tests/dashboard/test_health_page.py` (headless render shows every metric; read-only connections; busy state when locked) · Depends on: T18, T21a · Review: `/code-review`.
...
**Parallel lanes** (separate worktrees, non-overlapping files): T1 → {T2, T4}; T2 → T3 (owner); T4 → {T5, T20, T21a}; T5 → T6 → {T7, T8}; T8 → T8b → T9; {T7, T8b} → T10; {T3, T9} → T11; {T3, T7, T8b} → T12; {T9, T10} → T13 → {T14, T15}; {T11, T12} → T16 → T17; {T15, T17} → T18 → T19 → T22; {T18, T21a} → T21; {T21, T22} → T23.
