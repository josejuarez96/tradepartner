# 0002. Trunk-based git workflow; PR for every change; owner merges
...
## Decision
Option 1, as specified in [git-workflow.md](../ways-of-working/git-workflow.md). Protection is enforced locally (pre-commit and pre-push hooks, Claude deny rules). A server-side ruleset is ready in `.github/rulesets/protect-main.json` for when the plan allows it.

## Consequences
- Good: every change on `main` has an issue, a PR, CI, and an owner merge.
