...
|---|---|---|
| Free plan returns **SIP** history for past dates | `daily_bars(["SPY"], 2024-01-02..05, feed=SIP)` returned four bars, `feed: sip`, volume 123,007,793 on 2024-01-02 (consolidated, not IEX's ~2.5%) | Caveat "feed conflict" closed in favour of S1/S20. `alpaca.historical_feed` defaults to `sip`; real-time stays IEX. |
...
**Backfill runtime estimate** (documented limits, not measured):
- Alpaca, 1,000 symbols, 2016 to date, daily bars: about 2.5 M bars; multi-symbol requests page at 10,000 bars, so about 250 requests at 200/min: **under 5 minutes**. Corporate actions: similar or less.
- EDGAR: quarterly full indexes 1994Q3 to date, about 130 files of a few MB each at 10 req/s: **minutes**, bandwidth-bound. Submissions plus company facts for about 6,000 issuers: about 12,000 requests at 10/s: **about 20 minutes**, or one download each of `submissions.zip` and `companyfacts.zip` (nightly rebuilds).
- Total: **well inside one overnight run**, so the plan's fallback (`--since` defaulting to three years) is not needed.
