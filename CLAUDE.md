...
```
Run lint, format and mypy before every push, and pytest too when the change touches `src/`, `tests/`, `scripts/`, `.github/workflows/`, `pyproject.toml` or `uv.lock`. CI applies the same pytest rule on PRs and runs the full suite on every push to main.
