...
- Phase 2 T5: seeded, deterministic fixture-universe generator (`scripts/make_fixture_universe.py`), 25 synthetic securities (2018-01..2021-12) covering every case in spec req 13 (truncated/near/clean-merger delistings, 25-NSE, non-common Form 25, exchange transfer, ticker change, ticker reuse, dual-class, three split variants, revised dividend, restated and stale shares facts, unclassifiable name, `snapshot_static` pre-2019 listing, holiday/half day, SPY/MTUM benchmarks), plus a control group (`PLAIN1`/`PLAIN2`/`CTRL1`) that actually satisfies ADR 0006 rules 6-7 at a shared T; `tests/fixtures/universe/*.csv` and README (#25).
