...
- Phase 2 T20: `Broker` interface (`Order`/`Fill`/`Position` pydantic models, `OrderStatus`) and the in-memory `FakeBroker` (idempotent submit, cancel, positions, fills-since, deterministic session-open fill / session-close expiry via caller-supplied timestamps, no-look-ahead order eligibility, atomic price validation). No risk logic (ADR 0003 rule 7); risk rules stay in the Phase 4 wrapper (PR #26).
